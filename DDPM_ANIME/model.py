...
class DiffusionModel(pl.LightningModule):
    def __init__(self, image_size, num_steps: int, ch: int, ch_mult: List[int], attn: List[int], num_res_blocks: int, dropout: float, lr=1e-3):
        ...
        self.posterior_variance = self.betas * (1. - self.alphas_cumprod_prev) / (1. - self.alphas_cumprod)   # second posterior variance setting
    ...
    def reverse_step(self, x_t, t, posterior_choice=0):
        """
    ...
    def reverse_process(self, x_t, show_process=False, img_nums=1, process_nums=10, save_path=None, posterior_choice=0):
        """
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_steps - 1, -1, -1):
                cur_x = self.reverse_step(cur_x, i, posterior_choice)
